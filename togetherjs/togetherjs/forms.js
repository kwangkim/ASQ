...
define(["jquery", "util", "session", "elementFinder", "eventMaker", "templating", "ot", "peers"], function ($, util, session, elementFinder, eventMaker, templating, ot, peers) {
  ...
  //Array to store the user and their respective marker
  // var markerArray = [];
  ...
  var AceEditor = util.Class({
    ...
    constructor: function (el) {
      ...
      this._peerChanged = this._peerChanged.bind(this);
      peers.on("new-peer identity-updated status-updated", this._peerChanged);
    },
    ...
    updateCursor: function(msg) {
      var cursor = msg.cursor;
        ...
        for(var index = 0; index < this.markerArray.length; index++) {
          var user = this.markerArray[index];
          if (user.id == msg.peer.id) {
            if (user.marker === undefined) {
              user.marker = this._editor().document.addMarker(range, 'ace_selection', drawMarker, false);
            } else {
              this._editor().document.removeMarker(user.marker);
              user.marker = this._editor().document.addMarker(range, 'ace_selection', drawMarker, false);
            }
          }
        }
        //Handle the array with the users marker
